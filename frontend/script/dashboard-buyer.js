...
async function loadRecommended() {
  try {
    ...
    const response = await fetch('${API_BASE_URL}/api/dbuyerrecommend/recommended');
    if (!response.ok) {
...
async function updateQuickStats() {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/api/payment/buyer_orders/${buyerId}`);
    if (!response.ok) {
...
async function loadWalletBalance() {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/api/wallets/${userId}`);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('confirm-topup')?.addEventListener('click', async () => {
    ...
    try {
      let response = await fetch('${API_BASE_URL}/api/wallets/topup', {
        method: 'POST',
...
async function fetchBuyerOrders() {
  ...
  try {
    let response = await fetch(`${API_BASE_URL}/api/payment/buyer_orders/${userId}`);
    if (!response.ok) {
